...
미래 값을 입력으로 쓰지 않도록, 각 기준 시각에서 그 시각까지의 관측만 사용한다.
...
def evaluate(rows: list[dict], hours: int = 12, split: float = 0.8) -> dict:
    ...
    for i in range(start_index, len(rows) - 1):
        ...
        target_history = [Observation(r["time"], r["pm25"], r["wind_direction"], r["wind_speed"])
        ...
        weather = [WeatherPoint(r["time"], r["wind_direction"], r["wind_speed"])
                   for r in rows[i + 1: i + 1 + hours]]     # 예보값 대신 실제 기상값 사용(근사)
...
def main() -> None:
    ...
    result = evaluate(rows, args.hours, args.split)
    print_report(result)
    if args.json:
