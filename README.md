...
- 기존 대기질 앱은 현재 측정값만 보여 줍니다. 오염이 어느 방향에서 오는지, 언제 나아지는지는 알려 주지 않습니다.
...
- 바람길 판정: 풍향 예보와 시설(산단·발전소·데이터센터 부지) 위치를 비교해 "시설 → 우리 동네" 방향으로 바람이 부는 시간대를 표시
- 환기·외출 추천 시간: 대기질, 풍향·풍속, 강수 예보를 결합
- AI 한 줄 브리핑
...
| 데이터 | 제공처 | 용도 |
|---|---|---|
| 단기예보 (풍향·풍속·기온·강수) | 기상청 (공공데이터포털) | 시민 모드 예보 |
| 종관기상관측(ASOS) 시간자료 (일사량·풍속·기온) | 기상청 | 산업 모드 발전량·냉각 추정 |
| 대기오염정보 (실시간 측정값) | 한국환경공단 에어코리아 (공공데이터포털) | 시민 모드 대기질 |
| 시설 위치 | 공개 자료 수기 입력 | 바람길 판정 기준점 |

계산 결과는 의사결정을 돕는 추정치입니다. 대기 확산 모델이나 실측 발전량을 대체하지 않으며, 산식과 가정을 화면에 공개합니다.
...
flowchart LR
  ...
  COL --> DB[(SQLite)]
  DB --> API[FastAPI]
  API --> CALC[바람길 판정 · RE100 계산]
  API --> LLM[AI 브리핑 · LLM]
  API --> WEB[React/Next.js · 시민·산업 모드]
  WEB -. 선택 .-> ST[SmartThings 공기청정기]
```

- 배포: 대회 제공 AWS 서버, 사용자 권한 Supervisor와 crontab `@reboot`
- 프론트엔드는 빌드한 정적 파일을 FastAPI가 서빙해 포트 하나로 운영합니다.
...
- `main`에 직접 푸시하지 않습니다. `feature/기능명` 브랜치 → Pull Request → 1인 이상 리뷰 → 병합
- PR 본문은 템플릿(무엇·왜·검증·영향)을 따릅니다.
...
배포 구조와 작업 규칙은 강사 예제 [charsyam/scnu-oss-advanced-track-example](https://github.com/charsyam/scnu-oss-advanced-track-example) (MIT)를 따랐습니다.
