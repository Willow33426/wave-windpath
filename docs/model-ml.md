...
이 수치는 가상 데이터에서 나온 결과이며 실제 서비스 성능이 아니다. 현재 wind rule 평가는 미래의 실제 풍향·풍속을 예보 대신 사용하므로, 운영 환경에서 동일한 입력 조건의 비교로 해석할 수 없다.

## 모델 저장과 제한

`save_models()`와 `load_models()`로 12개 모델을 저장·불러올 수 있다. `predict(features)`는 기본 경로 `data/model.joblib`에서 모델을 찾고, 모델 파일이나 필요한 과거 관측이 없으면 기존 기준선으로 대체한다.
